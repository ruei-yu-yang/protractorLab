...
fdescribe('add a new event', () => {
  ...
  it('should type in an address', async () => {
    ...
    expect(await address.getAttribute('value')).toEqual('中正路100號');
  });

  it('should type in a city', async () => {
    await city.sendKeys('台北市');
    expect(await city.getAttribute('value')).toEqual('台北市');
  });
  ...
  it('should type in a online url', async () => {
    ...
    expect(await onlineUrl.getAttribute('value')).toEqual('http://example.com');
  });
